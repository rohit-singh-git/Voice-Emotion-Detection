...
class EmotionDetector:
    def __init__(self, model_name="E:/Model/Emotion Recognition/"):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        st.write(
                            f"DEBUG: Loaded converted file, shape: {audio_array.shape}, sample_rate: {sample_rate}")
                        # Clean up
                        os.remove(output_path)
                    except Exception as ffmpeg_error:
                        st.write(f"DEBUG: ffmpeg error: {str(ffmpeg_error)}")
                        return {
                            "error": f"Could not load audio file with any available method. Original error: {str(sf_error)}"}
...
def generate_image_local(emotion):
    prompt_map = {
    ...
    if "sd_pipe" not in st.session_state:
        with st.spinner("Loading Stable Diffusion model..."):
            ...
            if not torch.cuda.is_available():
                st.warning("⚠️ Running on CPU — image generation may be slow.")

            dtype = torch.float16 if use_cuda else torch.float32
            device = "cuda" if use_cuda else "cpu"

            pipe = StableDiffusionPipeline.from_pretrained(
                "E:/Model/Image Generation/",
                torch_dtype=dtype
            )

            pipe.to(device)
            st.session_state.sd_pipe = pipe
    else:
    ...
    image.save(f"{save_path}{emotion}_{time}_image.png")
    return image
...
def display_results(result, response_generator):
    ...
    st.subheader("AI Image for Emotion:")
    if st.button("🎨 Generate Art"):
        with st.spinner("Generating image..."):
            image = generate_image_local(result["predicted_emotion"])
            st.image(image, caption=f"Emotion: {result['predicted_emotion'].capitalize()}")
